...
http.listen(3000, function () {
	...
	mongoClient.connect("mongodb://localhost:27017", function (error, client) {
		var database = client.db("my_network");
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					if (request.files.coverPhoto.size > 0 && request.files.coverPhoto.type.includes("image")) {
						...
						coverPhoto = "public/userData/images/" + new Date().getTime() + "-" + request.files.coverPhoto.name;
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					if (request.files.profileImage.size > 0 && request.files.profileImage.type.includes("image")) {
						...
						profileImage = "public/userData/images/" + new Date().getTime() + "-" + request.files.profileImage.name;
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					if (request.files.image.size > 0 && request.files.image.type.includes("image")) {
						image = "public/userData/images/" + new Date().getTime() + "-" + request.files.image.name;
					...
					if (request.files.video.size > 0 && request.files.video.type.includes("video")) {
						video = "public/userData/videos/" + new Date().getTime() + "-" + request.files.video.name;
